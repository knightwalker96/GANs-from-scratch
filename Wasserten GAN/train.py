...
def main():
    ...
    gen = Generator(Z_DIM, CHANNELS_IMG, FEATURES_GEN, use_instance_norm = use_instance_norm).to(device)
    critic = Discriminator(CHANNELS_IMG, FEATURES_DISC, use_instance_norm = use_instance_norm).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (real, _) in tqdm(enumerate(loader), total=len(loader), desc=f"Epoch {epoch+1}/{args.epochs}"):
            real = real.to(device)
